...
function tryHost(url) { try { return new URL(url).hostname.replace('www.','') } catch { return 'feed' } }
function tryDate(d) { const t = Date.parse(d); return Number.isFinite(t) ? t : null }
...
newsRouter.get('/stream', async (req, res) => {
  const category = String(req.query.category || 'uplifting')
...
// store user prefs for personalization later
newsRouter.post('/prefs', (req, res) => {
