...
import { Send, Bot, AlertTriangle, HeartHandshake, PhoneCall, X, Sparkles, Volume2 } from 'lucide-react'
// Replaced heuristic arousal with HF emotion model
...
// Linkify while preserving line breaks
...
export default function Chat(){
  ...
  const baseLabel = (pick==='adem'?'🧠 Adem':'🌿 Eve')
  // Start with a short, playful exchange between the bot and a friendly cat persona.
  // We don't ask the user a question here — the user should message first.
  const [msgs, setMsgs] = useState<Msg[]>([{
    from: 'bot',
    text: `${pick==='adem' ? '🐱 Whiskers the cat' : '🐱 Whiskers the cat'}: Meow! Hi there.`,
    ts: Date.now() - 1000
  }, {
    from: 'bot',
  ...
  // TTS starts disabled — we only speak replies after the user explicitly activates voice
  ...
  // Auto-concern: if webcam shows sustained negative/high-arousal emotion, proactively offer help.
  useEffect(()=>{
    try {
      if (!moodTrail.length || !webEmotion?.label) return
      // do not auto-concern if camera backend is down
      if (cameraApiHealthy === false) return
      // consider labels 'sad','angry','fear','disgust' as negative
      ...
      const now = Date.now()
      // require at least 2 negative readings in the last 4, confidence decent, and not recently auto-responded
      if (negCount >= 2 && webEmotion.confidence && webEmotion.confidence >= 0.55) {
        if (!lastAutoRespond || (now - lastAutoRespond) > 1000 * 60 * 3) { // 3 minutes cooldown
          // Avoid auto-respond during a crisis (Chat.tsx handles crisis detection separately)
  ...
  const base = baseLabel
      ...
      // Build a history that includes the current user message to avoid repeating greeting-first logic
  ...
  const useLocal = ((import.meta as any).env?.VITE_LOCAL_LLM === 'true') || !!(import.meta as any).env?.VITE_LOCAL_LLM_PATH
      ...
      // Quick utility intent: "what's my expression" → report current webcam emotion
      ...
      // Domain restriction: only respond to mental wellbeing topics
        if (!isMentalHealthRelated(text)){
          ...
          // Avoid repeating the exact same boundary message — offer a short acknowledgement instead
          ...
          // If Gemini is available, ask it to humanize/rephrase the boundary (still refusing non-mental-health requests)
          if (hasGemini || useLocal){
            try {
              ...
              // fall through to default boundary
            }
          }
  ...
  // Handle transcripts coming from the browser STT component
  function handleTranscript(t: string){
    const trimmed = t.trim()
    if (!trimmed) return
    // If not mental-health related, show boundary instead of sending
    ...
    // urgent crisis check — if found, surface crisis modal immediately
    if (isCrisis(trimmed)){
      setMsgs(m => [...m, { from:'user', text: trimmed, ts: Date.now() }])
      setCrisis(trimmed)
      return
    }
    // If auto-send is enabled, send directly; otherwise populate input for review
    if (sttAutoSend) send(trimmed)
    else setInput(trimmed)
  }

  // Speak bot replies using SpeechSynthesis, but mute during crisis or when TTS disabled
  useEffect(()=>{
    try {
      // Only speak if TTS is enabled AND the user has explicitly activated voice (clicked the mic)
      if (!ttsEnabled || !voiceActive) return
      const last = msgs[msgs.length - 1]
      if (!last || last.from !== 'bot') return
      if (crisis) return // never TTS crisis modal
      const text = last.text || ''
      // Avoid speaking long helpline lists or phone numbers
  ...
  // Speak a single message on user demand (per-message play) with safety checks
  function speakMessage(text: string){
    try{
      if (!text) return
      // never read crisis/helpline lists
      if (crisis) return
      if (/\d{3,}/.test(text) && /helpline|call|emergency|phone|call\s+112/i.test(text)) return
      const synth: any = (window as any).speechSynthesis
      if (!synth) return
        ...
        <div className="flex items-center justify-between px-4 md:px-6 py-3 border-b border-slate-200/60 dark:border-slate-800/60 bg-white/60 dark:bg-slate-900/60">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <VoiceControls onTranscript={handleTranscript} autoSend={sttAutoSend} lang={navigator.language || 'en-US'} onStart={()=>{ setVoiceActive(true); setTtsEnabled(true) }} onStop={()=>setVoiceActive(false)} />
              <button onClick={()=>{ setTtsEnabled(prev=>{ const next = !prev; if (!next) setVoiceActive(false); return next }) }} title={ttsEnabled ? 'Disable voice replies' : 'Enable voice replies'} className={`text-xs px-2 py-1 rounded-md border ${voiceActive ? 'ring-2 ring-cyan-400' : ''}`}>
                {ttsEnabled ? '🔊' : '🔈'}
              </button>
            </div>
            {moodTrail.length > 0 && (
      ...
      <AnimatePresence>
        {!onboarding && (
          <OnboardingWizard onComplete={(ans)=>{
            saveOnboarding(ans)
            setOnboarding(ans)
            // personalize the two starter messages (cat and bot) without prompting the user to answer
            setMsgs(prev=>{
              if (prev.length >= 2){
                const m0 = prev[0]!
                const m1 = prev[1]!
                const updated0: Msg = { from: m0.from, ts: m0.ts, text: `${pick==='adem' ? '🐱 Whiskers the cat' : '🐱 Whiskers the cat'}: Meow! Hi${ans.name ? ' ' + ans.name : ''}.` }
