...
export default function OnboardingWizard({ onComplete }: Props){
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div role="dialog" aria-modal className="card max-w-2xl w-full rounded-2xl border border-slate-200/60 dark:border-slate-800/60 bg-white/80 dark:bg-slate-900/70 p-5 overflow-y-auto max-h-[88vh]">
        ...
        <div className="space-y-4 text-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="range" min={1} max={5} value={a.stress||3} onChange={e=>setA({...a, stress: Number(e.target.value) as any})} className="w-full" />
              <div className="text-xs mt-1">{a.stress||3} / 5</div>
            </div>
            <div>
              <label className="block text-xs mb-1">How was your sleep last night?</label>
              <input type="range" min={1} max={5} value={a.sleep||3} onChange={e=>setA({...a, sleep: Number(e.target.value) as any})} className="w-full" />
              <div className="text-xs mt-1">{a.sleep||3} / 5</div>
            </div>
          </div>
